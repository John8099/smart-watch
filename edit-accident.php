...
<body>
  ...
  <main id="main" class="main">
    <div class="card">
      <div class="card-body">
        ...
        <?php $proneDetails = $data["prone"][$_GET['i']] ?>
        <form class="p-4">
          <div class="col-12 mb-3">
            <label for="yourName" class="form-label"> Location</label>
            <input type="text" class="form-control" value="<?= $proneDetails["location"] ?>" required>
          </div>

          <div class="col-12 mb-3">
            <label for="yourName" class="form-label"> Coordinates</label>
            <input type="text" class="form-control" value="<?= $proneDetails["coordinates"] ?>" required>
          </div>

          <div class="col-12 mb-3">
            <label for="yourName" class="form-label">Description</label>
            <input type="text" class="form-control" value="<?= $proneDetails["description"] ?>" required>
          </div>

          <div class="col-12 mb-3">
            <label for="yourName" class="form-label">Accident count</label>
            <input type="text" class="form-control" value="<?= $proneDetails["accident_count"] ?>" required>
          </div>
  ...
  <script>
    ...
    let newCoord = [];
    const oldCoord = "<?= $proneDetails["coordinates"] ?>".trim().split(",");
    newCoord.push(Number(oldCoord[0]));
    newCoord.push(Number(oldCoord[1]));
